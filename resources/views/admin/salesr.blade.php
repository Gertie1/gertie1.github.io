            ...
            <div class="row">
                ...
                <div class="panel-body col-lg-11">
                    ...
                    <div class="panel-body col-lg-12">
                        <form role="form" method="post" action="{{ URL::route('newSale') }}" id="addstock">
                            {{ csrf_field() }}
                                ...
                                <tr>
                                    ...
                                    <div class="form-group">
                                            ...
                                            <select class="form-inline input-sm " name="batch_number[]" id="batch_number">
                                                ...
                                                @foreach($batch_numbers as $key => $batch_number)
                                    ...
                                    <td>
                                        <div class="form-group">
                                            <select class="form-inline input-sm " name="unit_price[]" id="unit_price">
                                                @foreach($drugs as $key => $drug)
                                                    {{--<option value="{{$drug->id}}"> {{$drug->name}}</option>--}}
                                                    <option value="{{$drug->sale_price}}"> {{$drug->sale_price}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </td>
        ...
        $(document).ready(function () {
            ...
            $('#add').click(function () {
                ...
                $('#table').append('<tr id="row'+i+'"><div class="form-group"><td><select class="form-inline input-sm " name="drug[]" id="drug"> @foreach($drugs as $key => $drug)
                                ...
                                </select></td></div><div class="form-group"><td><select class="form-inline input-sm " name="batch_number[]" id="batch_number">@foreach($batch_numbers as $key => $batch_number)
                        <option value="{{$batch_number->batch_number}}"> {{$batch_number->batch_number}}</option> @endforeach
                        </select></td></div><td><div class="form-group"><select class="form-inline input-sm " name="unit_price[]" id="unit_price">@foreach($drugs as $key => $drug)
                        {{--<option value="{{$drug->id}}"> {{$drug->name}}</option>--}}
                        <option value="{{$drug->sale_price}}"> {{$drug->sale_price}}</option>@endforeach</select></div></td><td><div class="form-group"><input class="input-sm" type="number" class="form-control" id="quantity_sold" name="quantity_sold[]" placeholder="Quantity sold"></div></td><td><div class="form-group"><input class="input-sm" type="date" class="form-control" id="complete_sold" name="complete_sold[]"placeholder="Date Sold"></div></td></td><td><button name="remove" id="'+i+'" class="btn btn-danger btn-remove">X</button></td></tr>');
